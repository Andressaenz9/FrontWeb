...
import { FooterArrendadorComponent } from "../../footer-arrendador/footer-arrendador.component";
...
@Component({
  ...
  standalone : true,
  imports: [CommonModule, FormsModule, NavbarArrendadorComponent, FooterArrendadorComponent],
})
export class EditarPropiedadComponent implements OnInit {
  ...
  constructor(
    ...
    private propiedadesService1: PropiedadesService,
  ...
  cargarPropiedad(): void {
    this.propiedadesService1.obtenerPropiedad(this.idPropiedad).subscribe(
      (data) => {
  ...
  guardarCambios(): void {
    if (this.validarPropiedad()) {
      ...
      this.propiedadesService1.editarPropiedad(this.idPropiedad, this.propiedad).subscribe(
        (response) => {
